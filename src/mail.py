...
def send_email(mail_subject, mail_content):
    ...
    """
    # 第三方 SMTP 服务，推荐163
    host = config.my_email_config['host']  # 设置服务器
    sender = config.my_email_config['sender']  # 用户名
    password = config.my_email_config['password']  # 口令
    receiver = config.my_email_config['receiver']  # 接收邮件，可设置为你的QQ邮箱或者其他邮箱
